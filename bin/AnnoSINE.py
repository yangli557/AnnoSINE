...
print('Example: python3 AnnoSINE.py 2 ../Input_Files/GCA_000001735.2_TAIR10.1_genomic.fasta ../Output_Files')
...
parser.add_argument("input_file_path", type=str, help="input genome assembly path")
parser.add_argument("output_file_path", type=str, help="output files path")
...
parser.add_argument("-l", "--length_factor", type=float, default=0.3,
                    help="Threshold of the local alignment length relative to the the BLAST query length (default: 0.3)")
parser.add_argument("-c", "--copy_number_factor", type=float, default=0.15,
                    help="Threshold of the copy number that determines the SINE boundary (default: 0.15)")
parser.add_argument("-s", "--shift",  type=int, default=80,
                    help="Maximum threshold of the boundary shift (default: 80)")
parser.add_argument("-g", "--gap",  type=int, default=10,
                    help="Maximum threshold of the truncated gap (default: 10)")
parser.add_argument("-minc", "--copy_number", type=int, default=3,
                    help="Minimum threshold of the copy number for each element (default: 3)")
...
parser.add_argument("-b", "--boundary", type=str, default='tsd',
                    help="Output SINE seed boundaries based on TSD or MSA (default: tsd)")
parser.add_argument("-m", "--model", type=str, default='rule',
                    help="Choose the rule/learning mode of repeat pattern examination (rule/learning) (default: rule)")
parser.add_argument("-f", "--figure", type=str, default='y',
                    help="Output the SINE seed MSA figures and copy number profiles (y/n) (default: n)")
...
def multiple_sequence_alignment(e_value, in_genome_assembly_path, out_genome_assembly_path):
    ...
    os.system('export PATH=$PATH:/Users/yolandali/Software/ncbi-blast-2.10.1+/bin'
              '&&echo $/Users/yolandali/Software/ncbi-blast-2.10.1+/bin'
              '&&blastn -query '+out_genome_assembly_path+'/Step2_extend_blast_input.fa '
...
def process_blast_output_1(in_genome_assembly_path, factor_length, factor_copy, max_shift, max_gap, min_copy_number,
    ...
    for m in range(len(hit_list)):
        ...
        if repeat_number > min_copy_number:
            if family_count[m] > 1:
                ...
                if hit_list[m][0] <= pos and hit_list[m][length[m] - 1] <= pos:
                    if len(blast_pre) == 1:
                        # shift
                        if abs(blast_pre[0]['start'] - 100) + abs(blast_pre[0]['end'] - (length[m] - 100)) >= max_shift:
                            update_pos.append({})
                        else:
                        ...
                        if len(blast_pre) - 2 == 0:
                            # gap
                            if abs(blast_pre[0]['end'] - blast_pre[1]['start']) >= max_gap:
                                max_gap_list.append(m)
                                del blast_pre[0]
                            if abs(blast_pre[0]['start'] - 100) + \
                            ...
                            for num_cut in range(0, len(blast_pre) - 2):
                                if abs(blast_pre[num_cut]['end'] - blast_pre[num_cut + 1]['start']) < max_gap:
                                    new_blast_pre.append(blast_pre[num_cut])
                            if len(new_blast_pre) == 0 or \
                                    abs(new_blast_pre[0]['start'] - 100) + \
...
def blast_rna(out_genome_assembly_path):
    os.system('export PATH=$PATH:/Users/yolandali/Software/ncbi-blast-2.10.1+/bin'
              '&&echo $/Users/yolandali/Software/ncbi-blast-2.10.1+/bin'
              '&&blastn -query '+out_genome_assembly_path+'/Step4_rna_input.fasta '
...
def process_rna(out_genome_assembly_path, model):
    rna_database = []
...
def tandem_repeat_finder(out_genome_assembly_path):
    ...
    os.system('chmod 775 /Users/yolandali/Software/trf409.macosx')
    os.system('/Users/yolandali/Software/trf409.macosx '
...
def inverted_repeat_finder(out_genome_assembly_path):
    path = os.path.abspath(os.path.dirname(os.getcwd()))
    os.system('/Users/yolandali/Software/irf305.macos.exe ' + out_genome_assembly_path +'/Step6_irf_input.fasta '
...
def cluster_sequences(out_genome_assembly_path):
    path = os.path.abspath(os.path.dirname(os.getcwd()))
    os.system('/Users/yolandali/Software/cdhit-master/cd-hit-est '
...
def re_process_figure(out_genome_assembly_path, figure):
    if figure == 'y':
        comp_1 = []
        comp_2 = []
...
def genome_annotate(in_genome_assembly_path, out_genome_assembly_path):
    path = os.path.abspath(os.path.dirname(os.getcwd()))
    os.system('/Users/yolandali/Software/RepeatMasker/RepeatMasker -e ncbi -pa 36 -q -no_is -norna -nolow -div 40 '
...
def main_function():
    ...
    input_genome_assembly_path = args.input_file_path
    output_genome_assembly_path = args.output_file_path
    ...
    if input_pattern == 1:
        ...
        hmm_predict(input_genome_assembly_path)
        sine_finder()
        process_sine_finder(input_genome_assembly_path, input_sine_finder, input_pattern, input_pattern)
        process_hmm_output_3(input_hmm_e_value, input_genome_assembly_path, input_pattern, output_genome_assembly_path)
    ...
    multiple_sequence_alignment(input_blast_e_value, input_genome_assembly_path, output_genome_assembly_path)
    if input_model == 'rule':
        process_blast_output_1(input_genome_assembly_path, input_factor_length, input_factor_copy_number,
                               ...
                               input_pos, output_genome_assembly_path, input_bound, input_figure)
        process_blast_output_2(output_genome_assembly_path)
    elif input_model == 'learning':
        print('TBD')
    ...
    process_rna(output_genome_assembly_path, input_model)
    print('========================= Step 4 has been done =======================\n\n')

    print('=============== Step 5: Tandem repeat finder has begun ===============')
    tandem_repeat_finder(output_genome_assembly_path)
    process_trf(trf_prob, output_genome_assembly_path)
    print('======================== Step 5 has been done ========================\n\n')
    ...
    print('========================= Step 6 has been done =======================\n\n')

    print('=============== Step 7: Sequences clustering has begun ===============')
    cluster_sequences(output_genome_assembly_path)
    print('======================== Step 7 has been done ========================\n\n')

    print('================= Step 8: Genome annotation has begun ================')
    re_process_figure(output_genome_assembly_path, input_figure)
    genome_annotate(input_genome_assembly_path, output_genome_assembly_path)
    print('========================= Step 8 has been done =======================\n\n')
